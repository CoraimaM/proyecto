    ...
    <div class="card mb-3">
        <div class="card-body">
            <form method="GET" action="{{ route('clientes.index') }}" class="row g-2 align-items-center">
                ...
                <div class="col-auto">
                    <button class="btn btn-outline-primary"><i class="fa fa-filter"></i> Filtrar</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end" style="display: flex; gap: 0.5rem; justify-content: flex-end; flex-wrap: wrap;">
                            <a href="{{ route('clientes.show', $cliente->id) }}" class="btn btn-sm" style="font-size: 0.8rem; white-space: nowrap; background-color: #fff; border: 1.5px solid var(--pink-dark); color: var(--pink-dark); font-weight: 500;"><i class="fa fa-eye me-1"></i>Ver</a>
                            <a href="{{ route('clientes.edit', $cliente->id) }}" class="btn btn-sm" style="font-size: 0.8rem; white-space: nowrap; background-color: #ffb3d9; border: 1.5px solid #ffb3d9; color: #fff; font-weight: 500;"><i class="fa fa-pen me-1"></i>Editar</a>
                            ...
                            @if(request()->get('show') === 'trashed' || (isset($show) && $show === 'trashed'))
                                ...
                                <form action="{{ route('clientes.forceDelete', $cliente->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Eliminar permanentemente?');">
                                    @csrf
                                    @method('DELETE')
                            ...
                            @else
                                <form action="{{ route('clientes.destroy', $cliente->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Eliminar (borrado lógico)?');">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="card-footer">
            <div class="d-flex justify-content-between align-items-center" style="flex-wrap: wrap; gap: 1rem;">
                <div class="text-muted" style="font-size: 0.8rem;">Mostrando {{ $clientes->firstItem() ?? 0 }} - {{ $clientes->lastItem() ?? 0 }} de {{ $clientes->total() }}</div>
                <div>
                    {{ $clientes->withQueryString()->links('vendor.pagination.custom-pagination') }}
                </div>
            </div>
        </div>
    </div>
