    ...
    <div class="card">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end" style="display: flex; gap: 0.5rem; justify-content: flex-end; flex-wrap: wrap;">
                            <a href="{{ route('productos.show', $producto->id) }}" class="btn btn-sm" style="font-size: 0.8rem; white-space: nowrap; background-color: #fff; border: 1.5px solid var(--pink-dark); color: var(--pink-dark); font-weight: 500;"><i class="fa fa-eye me-1"></i>Ver</a>
                            <a href="{{ route('productos.edit', $producto->id) }}" class="btn btn-sm" style="font-size: 0.8rem; white-space: nowrap; background-color: #ffb3d9; border: 1.5px solid #ffb3d9; color: #fff; font-weight: 500;"><i class="fa fa-pen me-1"></i>Editar</a>
                            ...
                            @if(request()->get('show') === 'trashed' || (isset($show) && $show === 'trashed'))
                                ...
                                <form action="{{ route('productos.forceDelete', $producto->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Eliminar permanentemente?');">
                                    @csrf
                                    @method('DELETE')
                            ...
                            @else
                                <form action="{{ route('productos.destroy', $producto->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Eliminar (borrado lógico)?');">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="card-footer">
            <div class="d-flex justify-content-between align-items-center" style="flex-wrap: wrap; gap: 1rem;">
                <div class="text-muted" style="font-size: 0.8rem;">Mostrando {{ $productos->firstItem() ?? 0 }} - {{ $productos->lastItem() ?? 0 }} de {{ $productos->total() }}</div>
                <div>
                    {{ $productos->withQueryString()->links('vendor.pagination.custom-pagination') }}
                </div>
            </div>
        </div>
    </div>
